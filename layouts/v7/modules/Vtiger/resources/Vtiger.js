...
//	showComposeEmailPopup : function(params, cb){
	showComposeEmailPopup : function(params, cb, step2){
				...
				if(params.search_module && params.search_module!= 'undefined') {
					thisInstance.searchModuleNames(params).then(function(data){
						...
						var serverDataFormat = data;
                                                ...
                                                //SalesPlatform.ru begin
						if(serverDataFormat.length <= 0 || serverDataFormat.success == false) {
						//if(serverDataFormat.length <= 0) {
								...
								serverDataFormat = new Array({
										...
										'type'	: 'no results'
                                                ...
                                                //SalesPlatform.ru begin
                                                if (serverDataFormat.result) {
                                                    for(var id in serverDataFormat.result){
                                                    //for(var id in serverDataFormat){
                                                    //SalesPlatform.ru end
                                                                    var responseData = serverDataFormat.result[id];
	...
	*/
	getReferenceSearchParams : function(element){
		...
		var searchModule = referenceModuleElement.val();
                ...
                //SalesPlatform.ru begin
                if (!searchModule && (element.attr('name')).slice(0, -1) == 'productName' ) {
