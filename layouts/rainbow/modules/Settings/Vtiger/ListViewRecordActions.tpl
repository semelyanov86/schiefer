...
<div class="table-actions" style = "width:60px">
    {foreach item=RECORD_LINK from=$LISTVIEW_ENTRY->getRecordLinks()}
        ...
        event.cancelBubble = true;{rdelim}" {else} href='{$RECORD_LINK_URL}' {/if}>
            <i class="{if $RECORD_LINK->getLabel() eq 'LBL_EDIT'}ti-pencil{else if $RECORD_LINK->getLabel() eq 'LBL_DELETE'}ti-trash{else}{$RECORD_LINK->getIcon()}{/if}" title="{vtranslate($RECORD_LINK->getLabel(), $QUALIFIED_MODULE)}"></i>
        </a>
        {if !$RECORD_LINK@last}
            &nbsp;&nbsp;
        {/if}
