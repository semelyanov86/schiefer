 ...
 * ****************************************************************************** */
 ...
 var supportedAdvancedCustomFields = {
    ...
    "Assigned_To": {'uitype': 53, 'name': "Assigned to", 'prefix': 'cf_acf_atf'},
        ...
        jQuery("textarea").each(function(){
            ...
            if(typeof is_rtf != "undefined" && view =="Edit") {
                if(is_rtf.substring(0, 10) === supportedAdvancedCustomFields.RTF_Description_Field.prefix){
                    var tr_parent = jQuery( this).parents('.row');
                    if(typeof tr_parent != "undefined") {
                        tr_parent.children( '.fieldValue:empty' ).remove();
                    }
        ...
        jQuery( "input[name^='" + supportedAdvancedCustomFields.Upload_Field.prefix + "']").each(function(){
            ...
            if(typeof is_upload_field != "undefined" && view =="Edit") {
                if(is_upload_field.substring(0, 10) === supportedAdvancedCustomFields.Upload_Field.prefix){
                    ...
                    var span_parent = jQuery( this).parents('.fieldValue');
        ...
        if(view == 'Edit'){
            jQuery( "input[name^='" + supportedAdvancedCustomFields.Upload_Field.prefix + "']").each(function(){
                ...
                if(html_content != "" && jQuery(this).attr('type') == "text"){
                    ...
                    if(res.length > 0){
                        var parent_span = jQuery( this).parent('.fieldValue');
                        ...
                        if(res[2].indexOf('image') > -1){
                            ...
                            var html = [
                                ...
                                '<input class="avfImageChange" type="hidden" value="0" name="imageChange['+jQuery( this).attr('name')+']">', // To check if image is changed when edit view
                                '</span>'
                            ].join('');
            ...
            jQuery("[id*='fieldValue_" + supportedAdvancedCustomFields.RTF_Description_Field.prefix + "']").each(function() {
                ...
                var parentFocus = focus.closest('.row');
                jQuery(this).closest('.fieldValue').prev().css('width','8%');
                ...
                if(inputOriginRTF.length == 0 ) {
                    ...
                    focus.closest('.row').append(newInput);
                }else{
...
jQuery(document).ready(function(){
    ...
    jQuery( document ).ajaxComplete(function(event, xhr, settings) {
        ...
        if (settings.data && settings.data.indexOf("mode=unHide") > -1){
...
function avcf_upload_files(is_upload_field){
    $("#frm_"+is_upload_field).append('<input id="onlyForUploadFile" type="hidden" value="' + is_upload_field + '" name="field_name">');
    ...
    jQuery.ajax({
        ...
        if(res.length > 0){
            var parent_span = jQuery(target_input).closest('.fieldValue');
            parent_span.find('img').remove();
            parent_span.find('span').remove();
            if(res[2].indexOf('image') > -1){
                var img = res[0].replace(/\s+/g," ").trim();
                var html = [
                    '<span>',
                    '<img style="width:150px;height:150px;" src="'+img+'" />',
                    '<input style="margin-left: 10px;" class="avfImageDelete" data-field_name = "'+is_upload_field+'" data-file="'+return_file+'" type="button" value="Delete" onclick="removeThis(this)">',
                    '<input class="avfImageChange" type="hidden" value="1" name="imageChange['+is_upload_field+']">', // To check if image is changed when edit view
                    '</span>'
                ].join('');
                parent_span.append(html);
            }
                ...
                var html = [
                    ...
                    '<input style="margin-left: 10px;" class="avfImageDelete" data-field_name = "'+is_upload_field+'"  data-file="'+return_file[0]+'" type="button" value="Delete" onclick="removeThis(this)">',
                    '<input class="avfImageChange" type="hidden" value="1" name="imageChange['+is_upload_field+']">', // To check if image is changed when edit view
                    '</span>'
                ].join('');
                parent_span.append(html);
...
function removeThis(btn){
    ...
    var parent_span = jQuery(btn).parent('span').parent('.fieldValue');
